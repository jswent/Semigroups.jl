...
// Clear the error log
void clear_error_log() { error_log.clear(); }

// Add an error message to the log
void log_error(const std::string &msg) { error_log.push_back(msg); }

// Check if there are any logged errors
bool have_error() { return !error_log.empty(); }
...
void define_errors(jl::Module &mod) {
  ...
  mod.method("have_error", &have_error);
  mod.method("get_and_clear_errors", &get_and_clear_errors);
  mod.method("clear_error_log", &clear_error_log);

  // Helper function to safely execute a function and catch exceptions
  // This is used internally by other binding functions
  mod.method("_try_catch_test", []() -> bool {
    try {
